...
// Factory returning an external pointer to a polymorphic WeightedBootstrap (raw pointer owned by XPtr)
// [[Rcpp::export]]
SEXP get_weighted_bootstrap_ptr_cpp(std::size_t N, std::size_t B, const std::string& type, Rcpp::Nullable<Rcpp::NumericVector> seed = R_NilValue) {
    ...
    apm::WeightedBootstrap* ptr = nullptr;
    if (t == "multinomial") {
        ptr = new apm::MultinomialBootstrap(N, B, s);
    } else if (t == "bayesian") {
        ptr = new apm::BayesianBootstrap(N, B, s);
    } else {
        Rcpp::stop("type must be either 'multinomial' or 'bayesian'");
    }

    // XPtr takes ownership and will delete ptr when GC'd
    return Rcpp::XPtr<apm::WeightedBootstrap>(ptr, true);
}

// Accessor wrappers operating on the pointer
// [[Rcpp::export]]
std::size_t wb_n_obs_cpp(SEXP xp) {
    Rcpp::XPtr<apm::WeightedBootstrap> p(xp);
    return p->n_obs();
}
